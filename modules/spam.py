...
#                self.tinybot.console_write(pinylib.COLOR['red'], '[Security] Lockdown started')
    ...
    def check_msg(self, msg):
        ...
        points = re.findall('SPENDPOINTS', msg)
        points2 = re.findall('spendpoints', msg)
        ...
        if self.config.B_SPAMP:
            ...
            if points:   # tricks
                self.tinybot.handle_msg("Purchase complete. You have bought one: SUICIDE NOW. Delivery in 10 seconds..(%s)!" % len(msg))
                spammer = True
                sleep(randint(9,10))
            ...
            for m in self.msgs:
                ...
                if msg == m and m not in self.general:
                    ...
                    if totalcopies > 0: # repeated message
                        ...
                        if totalcopies > 1: # if copy exists more than 5 times
                            #self.autoban_time = time.time()
                            #self.do_lockdown(1)
                            ban = True
