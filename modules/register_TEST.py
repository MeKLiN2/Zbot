    ...
    def on_publish(self, uid):
        ...
        if self.config.B_VIP and not buddyusr:
            if _user is None:
                ...
                self.handle_msg('%s TYPE: !ACC ADD NAME123 VERIFIED' % _user.nick)
...
#timer by meklin 5-12 lets them cam up a few seconds
#change it to 3-9 or 4-8 or so to make it look human and be fast
#5-12 is being generous
#Rename register.py to register_OLD.py
#replaced with a sneakier and human-like timer for
#!VIP mode ban, to trick persistant idiots into wasting their computing power all day.
#Make sure to run a video of you typing furiously on loop.
#samejeff destroyer script
                    _user.user_level = 8
