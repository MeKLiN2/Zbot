...
class TinychatBot(pinylib.TinychatRTCClient):
    ...
    def cmd_handler(self, msg):
        ...
        elif cmd == prefix + 'hitler':
            self.do_hitler_quote(cmd_arg)
            
        elif cmd == prefix + 'mirror':
            self.do_magicmirror(cmd_arg)

        elif cmd == prefix + 'tarot':
            self.do_tarot(cmd_arg)
    ...
    def do_tarot(self, question):
        ...
        if len(question) is 0:
            self.send_chat_msg('TarotCards %s' % locals_.tarot_cards())
        else:
            self.send_chat_msg('TarotCards %s' % locals_.tarot_cards())

    def do_hitler_quote(self, question):
        self.send_chat_msg('%s' % locals_.hitler_quote())
            
    def do_magicmirror(self, question):
        """
        magic mirror by meklin for qwerty
        :param question: The yes/no question.
        :type question: str
        """
        if len(question) is 0:
            ...
            self.send_chat_msg('MagicMirror %s' % locals_.magicmirror())
